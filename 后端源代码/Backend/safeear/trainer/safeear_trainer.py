...
class SafeEarTrainer(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        csv_path = "score.csv"
        if all_score[0] < 0.5:
            all_index = np.array([0])
        else:
            all_index = np.array([1]) 
            
        with open(csv_path, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            for (fn,), sc, lbl in zip(all_filename, all_score, all_index): # FIX
                print(f"[Debug]: all_index:{all_filename} type:{type(all_filename)}")
                print(f"[Debug]: all_index:{all_score} type:{type(all_score)}")
                print(f"[Debug]: all_index:{all_index} type:{type(all_index)}")
                writer.writerow([fn, f"{sc:.6f}", int(lbl)])
