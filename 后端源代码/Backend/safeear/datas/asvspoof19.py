...
class ASVSppof2019(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train == False and audio.shape[1] > self.max_len:
            ...
            feat_st = 0
            ed = st + self.max_len
            slice_feat = avg_hubert_feat[:, feat_st:feat_st + feat_duration]
            if slice_feat.shape[1] < feat_duration:
                slice_feat = torch.nn.functional.pad(
                    slice_feat, (0, feat_duration - slice_feat.shape[1]), "constant", 0
                )
            # 始终带上 audio_path
            return audio[:, st:ed], slice_feat, target, str(audio_path)
        ...
        if audio.shape[1] < self.max_len:
            ...
            if avg_hubert_feat.shape[1] < feat_duration:
                avg_hubert_feat = torch.nn.functional.pad(
                    avg_hubert_feat,
                    (0, feat_duration - avg_hubert_feat.shape[1]),
                    "constant",
                    0
                )
        if avg_hubert_feat.shape[1] < feat_duration:
